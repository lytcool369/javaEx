...
//		searchEx();
	...
	private static void arrayCopyEx() {
		...
		target = Arrays.copyOfRange(src, 4, 8);
	...
	private static void searchEx() {
		...
		// 사용자 정의 객체의 검색: Comparable이 구현되어 있어야 함
		Member[] members = {
				new Member("임꺽정"),
				new Member("홍길동"),
				new Member("고길동")
	...
	private static void custiomSort() {
		// 사용자 정의 객체의 배열
		Member[] members = {
				new Member("임꺽정"),
				new Member("홍길동"),
		...
		// 원본 출력
		System.out.println("원본 배열: " + Arrays.deepToString(members));
		...
		System.out.println("역순 정렬된 배열: " + Arrays.deepToString(members));
	}
	private static void basicReverseSort() {
		...
		// 역순 정렬 (내림차순)
		Arrays.sort(scores, Collections.reverseOrder());	// 역순 정렬
		System.out.println("역순 정렬된 배열: " + Arrays.toString(scores));	
	}
	...
	private static void basicSort() {
		...
		// 정렬 (오름차순)
		Arrays.sort(scores);
		System.out.println("정열된 배열: " + Arrays.toString(scores));	
	}
}
