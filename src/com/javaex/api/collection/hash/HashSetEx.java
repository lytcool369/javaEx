...
public class HashSetEx {
	...
	private static void customTypeSet() {
		...
		// 1. hashCode() 값 비교
		// 2. equals() 결과 확인
		System.out.println("s2 s3 의 해시코드 값이 같은가? " + (s2 == s3));
		System.out.println("s2 s3 의 해시코드 값이 같은가? " + (s2.hashCode() == s3.hashCode()));
		System.out.println("s2 s3 의 값이 같은가? " + (s2.equals(s3)));
		
		// 학생 임꺽정이 포함되어 있는가?
		System.out.println("학생 임꺽정이 포함되어 있는가? " + cls.contains(s2));
