...
//		guestbookUpdate();
	...
	private static void guestbookUpdate() {
		// 연습문제
		// 방명록 번호, 비밀번호, 내용을 입력받아 Update
		guestbookSelectAll();
		GuestbookDao dao = new GuestbookDaoImpl();
		Scanner scanner = new Scanner(System.in);
		
		System.out.print("수정할 방명록 번호: ");
		...
		// DTO 생성
		GuestbookVo vo = new GuestbookVo(no, null, password, message, null);
		
		int updatedCount = dao.update(vo);
		
		System.out.println(updatedCount + "개의 레코드가 수정되었습니다.");
	...
	private static void guestbookDelete() {
		...
		System.out.println(deletedCount + "개의 레코드가 삭제되었습니다.");
	...
	private static void selectTest() {
		// employees 데이터베이스의 employees 테이블로부터
		// 모든 레코드를 받아서 출력
		String dburl = "jdbc:mariadb://192.168.1.137:3307/employees?userSSL=false";
		...
		try {
			...
			// 5. 결과 처리
			while(rs.next()) {	// 커서에서 다음 레코드를 아오기
				// 컬럼 데이터 받아오기
				Long emp_no = rs.getLong(1);			// 인덱스 값이 1부터 시작함
				String firstName = rs.getString("first_name");	// 문자 인덱스(컬럼명)
				String lastName = rs.getString("last_name");
				Date birthDate = rs.getDate(4);		// java.util.Date
				
				SimpleDateFormat sdf = new SimpleDateFormat("YYYY-mm-dd");
				System.out.printf("%d - %s %s (생일: %s)%n", emp_no, firstName, lastName, sdf.format(birthDate));
			} 
